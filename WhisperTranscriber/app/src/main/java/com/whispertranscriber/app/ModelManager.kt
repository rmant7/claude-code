...
class ModelManager(private val context: Context) {
    ...
    /**
     * Streams the model into a ".part" temp file and only promotes it to the final file once the
     * whole download has been verified complete (see the DownloadManager truncation bug this
     * replaced). Large models on flaky connections routinely hit read timeouts mid-transfer, so
     * this resumes via HTTP Range requests from wherever the ".part" file left off and retries
     * with backoff, as long as each attempt makes forward progress — it only gives up after
     * [MAX_CONSECUTIVE_FAILURES] attempts in a row that transferred zero new bytes (e.g. no
     * network at all, or the server rejecting the request outright).
     */
    fun downloadModel(model: WhisperModel, onProgress: (Progress) -> Unit) {
        ...
        while (consecutiveFailures < MAX_CONSECUTIVE_FAILURES) {
            ...
            try {
                downloadOnce(model.url, tempFile, onProgress)
                destination.delete()
